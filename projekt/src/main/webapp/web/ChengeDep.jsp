...
<h1>Изменение отделов на предприятии</h1>

    <form action="<c:url value="/changedep"/>"method="post">
     <label for="iddep">№ отдела</label>
    <select name="iddep"><option></option><c:forEach var="item" items="${departaments}">
        <option>${item.getIdDep()}</option></c:forEach></select><br>
...
<c:if test="${not empty changedep}">
<div>
<table >
<caption><b><i>Изменен отдел на предприятии</i></b></caption>
<thead>
<tr>
<th>id отдела</th>
...
<tbody>
<c:forEach var="item" items="${changedep}" >
<tr>
<td>${item.getIdDep()}</td>
