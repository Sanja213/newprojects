...
<h1>Просмотр данных по должностям на предприятиии</h1>

    <form  action="<c:url value="/position"/>"method="post">
      <select name="iddep"><option></option><c:forEach var="item" items="${positions}">
        <option>${item.getId()}</option></c:forEach></select>
      <select name="namedep"><option></option><c:forEach var="item" items="${positions}">
        <option>${item.getName()}</option></c:forEach></select>
      <select name="tarif"><option></option><c:forEach var="item" items="${positions}">
        <option>${item.getTarif()}</option></c:forEach></select>
...
<c:if test="${not empty posout}">
<div class="tab">
<table >
<caption><b><i>Список должностей на предприятии</i></b></caption>
<thead>
<tr>
<th>id должности</th>
...
<tbody>
<c:forEach var="item" items="${posout}" >
<tr>
<td>${item.getId()}</td>
<td>${item.getName()}</td>
<td>${item.getTarif()}</td>
